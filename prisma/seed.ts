...
async function main() {
  ...
  const roles = [
    ...
    { code: 'DA', name: 'Admin Doxart', description: 'Obsługa archiwum i spisów wszystkich klientów', isSystem: true, permissions: ['tenant.switch', 'box.read', 'box.write', 'box.delete', 'box.move', 'box.status', 'folder.read', 'folder.write', 'document.read', 'document.write', 'location.read', 'location.write', 'order.read', 'order.create', 'order.approve', 'order.process', 'order.complete', 'hr.view', 'hr.write', 'hr.view_pesel', 'label.read', 'label.generate', 'label.template_manage', 'attachment.read', 'attachment.upload', 'attachment.delete', 'search.all', 'search.own', 'report.view', 'report.export', 'audit.view', 'retention.manage', 'disposal.initiate', 'import.data', 'export.data', 'inventory.manage', 'transfer_list.read', 'transfer_list.write', 'transfer_list.import'] },
    { code: 'TL', name: 'Szefostwo tenanta', description: 'Dostęp do całej dokumentacji własnej firmy oraz edycja Spisów ZO', isSystem: true, permissions: ['box.read', 'folder.read', 'document.read', 'order.read', 'order.create', 'order.approve', 'order.process', 'order.complete', 'hr.view', 'attachment.read', 'search.own', 'report.view', 'transfer_list.read', 'transfer_list.write'] },
    { code: 'TE', name: 'Pracownik tenanta', description: 'Dostęp do dokumentacji przypisanego działu', isSystem: true, permissions: ['box.read', 'folder.read', 'document.read', 'order.read', 'order.create', 'attachment.read', 'search.own'] },
  ...
  const tlRole = await prisma.role.findFirst({ where: { code: 'TL', tenantId: null } });
  if (tlRole) {
    await prisma.userRole.upsert({
      where: { userId_roleId: { userId: tenantAdmin.id, roleId: tlRole.id } },
      update: {},
      create: { userId: tenantAdmin.id, roleId: tlRole.id },
    });
  }
  console.log('  ✅ Tenant Leadership: admin@demo.pl / Admin123!@#');
